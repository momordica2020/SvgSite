...
def is_duplicate_text(new_text, existing_texts, min_overlap=0.7):
    """检查文本是否与已有文本重复（包括子串包含关系）"""
    ...
    for old in existing_texts:
        if not old or len(old) < 20:
            continue
        old_lower = old.lower()
        old_len = len(old_lower)
        if new_lower == old_lower:
            return True
        if new_len > old_len and old_lower in new_lower:
            return True
        if old_len > new_len and new_lower in old_lower:
            return True
    return False


def parse_with_bs4(html_content, code):
    """使用BeautifulSoup解析 - 支持多级标题、段落去重"""
    ...
    for bad in main_soup.find_all(['script', 'style', 'noscript', 'hr', 'br']):
        bad.decompose()
    ...
    ratio_match = re.search(r'(\d+)\s*:\s*(\d+)', main_html[:3000])
    if ratio_match:
        result["flag_ratio"] = ratio_match.group(0)

    all_headings = []
    heading_tags = main_soup.find_all(['h1', 'h2', 'h3', 'h4'])
    for heading in heading_tags:
        text = clean_text(heading.get_text())
        if text and len(text) < 100:
            level = int(heading.name[1])
            text_lower = text.lower().strip(':').strip()
            if text_lower in ('navigation', 'nav', 'see also', 'see:'):
                continue
    ...
    see_also_patterns = [
        r'(?:<em>|<i>|<strong>|<b>)?\s*(?:See also|See also:|See:)\s*:?\s*(?:</em>|</i>|</strong>|</b>)?',
        r'<h[1-6][^>]*>\s*(?:See also|See also:)\s*</h[1-6]>',
    ]
    for pattern in see_also_patterns:
        see_also_match = re.search(pattern, main_html, re.IGNORECASE)
        if see_also_match:
            ...
            next_h = re.search(r'<h[1-6][^>]*>', after_text, re.IGNORECASE)
            end_pos = len(after_text)
            if next_hr:
                end_pos = min(end_pos, next_hr.start())
            if next_h:
                end_pos = min(end_pos, next_h.start())
        ...
        if see_also_start_line is not None:
            if el_line >= see_also_start_line:
                if see_also_end_line is None or el_line < see_also_end_line:
                    return True
        if el_text and re.match(r'(see also|see:)', el_text.lower()[:30]):
            return True
        return False
    ...
    def add_paragraph_to_section(sec_idx, text):
        ...
        if text_hash in sec["_seen_paras"]:
            return
        if is_duplicate_text(text, sec["paragraphs"]):
            return
        sec["_seen_paras"].add(text_hash)
        sec["paragraphs"].append(text)

    def add_image_to_section(sec_idx, src, alt):
        if sec_idx < 0 or sec_idx >= len(sections):
            return
        ...
        sec = sections[sec_idx]
        for existing in sec["images"]:
            if existing["src"] == src:
                return
        sec["images"].append(img_info)

    for element in main_soup.find_all(['p', 'blockquote', 'pre', 'div']):
        sec_idx = get_current_section_idx(element)
        ...
        if tag_name == 'blockquote':
            ...
            if text and len(text) > 20:
                ...
                for img in element.find_all('img'):
                    src = normalize_img_src(img.get('src', ''))
                    alt = clean_text(unescape(img.get('alt', '')))
                    add_image_to_section(sec_idx, src, alt)
                if sec_idx >= 0:
                    sec = sections[sec_idx]
                    q_hash = hash(text[:100].lower())
                    if q_hash not in sec.get("_seen_paras", set()):
                        if not is_duplicate_text(text, sec["quotes"]):
                            sec["quotes"].append(text[:1500])
            continue

        text = clean_text(element.get_text())

        has_only_imgs = len(element.find_all('img')) > 0 and len(text) < 50
        if has_only_imgs and (tag_name == 'p' or tag_name == 'div'):
            for img in element.find_all('img'):
                src = normalize_img_src(img.get('src', ''))
                alt = clean_text(unescape(img.get('alt', '')))
                add_image_to_section(sec_idx, src, alt)
        ...
        if tag_name == 'pre':
            if text:
                add_paragraph_to_section(sec_idx, text)
            continue
        ...
        if re.match(r'^(image by|by\s|Image by|\d+:\d+\s*image by|\d+:\d+\s*$)', text, re.I) and len(text) < 150:
            for img in element.find_all('img'):
                src = normalize_img_src(img.get('src', ''))
                alt = clean_text(unescape(img.get('alt', '')))
                add_image_to_section(sec_idx, src, alt)
        ...
        if text and len(text) >= 15:
            text_clean = re.sub(r'^\s*\d+:\d+\s*(;|,|\|)?\s*(image by|by\s|Image by[^.]+\.?\s*)?', '', text, flags=re.I)
            text_clean = re.sub(r'^(image by|by\s|Image by[^.]+\.?\s*)', '', text_clean, flags=re.I)
            text_clean = re.sub(r'^(Flag of [^-]+ - Image by [^,]+,\s*\d+\s+\w+\s+\d+\s*)', '', text_clean, flags=re.I)
            add_paragraph_to_section(sec_idx, text_clean)
    ...
    for p in main_soup.find_all('p'):
        ...
        if text and len(text) > 80 and not re.match(r'^(image by|\d+:\d+)', text, re.I):
    ...
    for list_tag in main_soup.find_all(['ul', 'ol']):
        ...
        if not is_nav_list and sec_idx >= 0 and sec_idx < len(sections):
            sec = sections[sec_idx]
            for item in items:
                if not is_duplicate_text(item, [p for lst in sec["lists"] for p in lst]):
                    pass
            sec["lists"].append(items[:20])
    ...
    for table in main_soup.find_all('table'):
        ...
        if is_nav:
            ...
            for img in table_images:
                add_image_to_section(sec_idx, img["src"], img["alt"])
            continue
        ...
        if table_text and len(table_text) > 100 and sec_idx >= 0 and sec_idx < len(sections):
            if not is_duplicate_text(table_text[:1000], sections[sec_idx]["paragraphs"]):
                add_paragraph_to_section(sec_idx, table_text)
    ...
    for i, sec in enumerate(sections):
        ...
        if i == 0:
            ...
            if paras:
                intro_clean = []
                for p in paras:
                    p_clean = re.sub(r'^\s*\d+:\d+\s*(;|,|\|)?\s*(image by|by\s|Image by[^.]+\.?\s*)?', '', p, flags=re.I)
                    p_clean = re.sub(r'^(image by|by\s|Image by[^.]+\.?\s*)', '', p_clean, flags=re.I)
                    p_clean = re.sub(r'^(Flag of [^-]+ - Image by [^,]+,\s*\d+\s+\w+\s+\d+\s*)', '', p_clean, flags=re.I)
                    p_clean = p_clean.strip()
                    if p_clean and len(p_clean) > 15:
                        if not is_duplicate_text(p_clean, intro_clean):
                            intro_clean.append(p_clean)
                if intro_clean:
                    intro_text = " ".join(intro_clean[:2])[:600]
    ...
    for code in test_codes:
        if code in details:
            ...
            for s in d.get('sections', []):
                ...
                print(f"    - {s['title']}(h{s.get('level',2)}): {n_paras}段落, {n_imgs}图片, {n_quotes}引用, {n_lists}列表")
